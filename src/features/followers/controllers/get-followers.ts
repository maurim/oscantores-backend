...
export class Get {
  public async userFollowing(req: Request, res: Response): Promise<void> {
    const userObjectId: any | ObjectId = new mongoose.Types.ObjectId(req.currentUser!.userId);
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Usuário seguindo', following });
  }

  public async userFollowers(req: Request, res: Response): Promise<void> {
    const userObjectId: any | ObjectId = new mongoose.Types.ObjectId(req.params.userId);
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Seguidores de usuários', followers });
  }
}
