...
import { INotificationSettings, ISocialLinks, IUserDocument } from '@user/interfaces/user.interfaces';
...
export class UserCache extends BaseCache {
  ...
  public async saveUserToCache(key: string, userUId: string, createdUser: IUserDocument): Promise<void> {
    ...
    try {
      ...
      log.error(error);
      throw new ServerError('Server error. SaveUserToCache Try again.');
    }
  }
  ...
  public async getUsersFromCache(start: number, end: number, excludedUserKey: string): Promise<IUserDocument[]> {
    try {
      ...
      log.error(error);
      throw new ServerError('Server error getUsersFromCache. Try again.');
    }
  }
