variable "aws_region" {
  ...
  type        = string
  default     = "sa-east-1" # Your AWS region
}
...
variable "vpc_availability_zones" {
  ...
  type        = list(string)
  default     = ["sa-east-1a", "sa-east-1b"] # Two availability zones for your specific region
}
...
variable "main_api_server_domain" {
  ...
  type        = string
  default     = "oscantores.xyz" # Your backend domain you created a route53 zone for
}

variable "dev_api_server_domain" {
  description = "Dev API Server Domain"
  type        = string
  default     = "api.dev.oscantores.xyz"
}

variable "ec2_iam_role_name" {
  description = "EC2 IAM Role Name"
  type        = string
  default     = "oscantores-server-ec2-role" # Add a unique name
}

variable "ec2_iam_role_policy_name" {
  description = "EC2 IAM Role Policy Name"
  type        = string
  default     = "oscantores-server-ec2-role-policy" # Add a unique name
}

variable "ec2_instance_profile_name" {
  description = "EC2 Instance Profile Name"
  type        = string
  default     = "oscantores-server-ec2-instace-profile" # Add a unique name
}
...
variable "code_deploy_role_name" {
  ...
  type        = string
  default     = "oscantores-server-codedeploy-role" # Add a unique name
}

variable "prefix" {
  description = "Prefix to be added to AWS resources tags"
  type        = string
  default     = "cantores-server" # Add a unique identifier name
}

variable "project" {
  description = "Prefix to be added to AWS resources local tags"
  type        = string
  default     = "cantores-server" # You can use the name unique identifier created above
}
